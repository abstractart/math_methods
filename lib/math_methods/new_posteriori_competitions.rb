class MathMethods::NewPosterioriCompetition
  ...
  def initialize(task, result_range, task_type)
    @task = task
    @result_range = convert_range(result_range)
  ...
  def convert_range(result_range)
    result = {}

    result_range.each do |range, alternatives|
      Array(alternatives).each do |alternative|
        result[alternative] = range
      end
    end
